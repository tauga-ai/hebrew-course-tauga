 ...
 * session/open-answer/route.ts's own inline version so a second (now third,
 * and role-play a fourth) graded topic doesn't each carry a slightly-drifting
 * copy of the same logic. A debate or role-play answer landing between two
 * open-topic answers must not silently break the streak in either direction,
 * so this merges all of them before computing it.
 *
...
export async function fetchRecentGradedAnswersMilestone(db: SupabaseClient, studentId: string): Promise<number | null> {
  const [{ data: openRows }, { data: debateRows }, { data: roleplayRows }] = await Promise.all([
    ...
    db.from('naale_roleplay_answers').select('score, session_id, answered_at').eq('student_id', studentId).eq('is_review', false).order('answered_at', { ascending: false }).limit(15),
  ])
  const recentAnswers: RecentAnswerRow[] = [...(openRows ?? []), ...(debateRows ?? []), ...(roleplayRows ?? [])]
