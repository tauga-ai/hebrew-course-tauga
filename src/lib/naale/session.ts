...
export { SESSION_MINUTES, TOPIC_SESSION_MINUTES, MIN_ANSWERS_FOR_COMPLETION, isSessionCompleted, hasReachedTimer, isExpired, isSessionExpired, secondsRemaining, isPendingQuestion, canPause, isPaused, isPauseExpired, isTopicMismatch, isPracticeConflict, remainingToBank, resumedDeadline, remainingMs } from './session-rules'
...
export async function loadOwnedSession(sessionId: string, studentId: string): Promise<OwnedSessionResult> {
  ...
  const { data } = await db
    .from('naale_sessions')
    .select('id, student_id, kind, topic, pending_question_id, started_at, deadline_at, ended_at, answered_count, completed, translations_used, translated_words, paused_remaining_ms, pending_exchange')
    .eq('id', sessionId)
    .maybeSingle()
