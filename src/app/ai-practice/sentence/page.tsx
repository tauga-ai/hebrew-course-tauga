...
const LEVEL_LABELS: Record<number, string> = {
  1: t('מילון יומיומי בסיסי: בית, משפחה, בית ספר'),
  2: t('פעולות ומצבים: עבודה, נסיעה, קנייה'),
  3: t('נושאים מגוונים: ספורט, מוזיקה, טיול'),
  4: t('אוצר מילים מתקדם: טכנולוגיה, חברה, מדע'),
  5: t('מושגים אקדמיים: ניתוח, השוואה, מסקנות'),
}
...
export default function AISentencePage() {
  ...
  async function generateExercise(lvl: number) {
    ...
    try {
      ...
      setError(t('שגיאה ביצירת התרגיל. נסה שוב.'))
      setPhase('pick')
    }
  }
  ...
  async function submitSentence() {
    ...
    try {
      ...
      alert(t('שגיאה בבדיקת המשפט. נסה שוב.'))
      setPhase('input')
    }
  }
  ...
  return (
    <div className="min-h-screen md:flex">
      ...
      {phase === 'pick' && (
        <>
          <p className="text-center text-fg/60 text-sm mb-2">{t('בחר רמה וה-AI יצור לך תרגיל בניית משפט')}</p>
          ...
          {scores.length > 0 && (
            <div className="mt-6 bg-black/5 dark:bg-white/5 rounded-xl p-4 text-center text-sm text-fg/70">
              {t('ממוצע כולל')}: {(scores.reduce((a,b)=>a+b,0)/scores.length).toFixed(1)}/10 ({scores.length} {t('תרגילים')})
            </div>
          )}
        </>
      )}
      ...
      {phase === 'gen-loading' && (
        <div className="flex flex-col items-center justify-center min-h-64 gap-4">
          ...
          <p className="text-fg/60">{t('יוצר תרגיל ברמה')} {level}...</p>
        </div>
      )}
      ...
      {phase === 'input' && wordList && (
        <>
          <div className="bg-primary-50 dark:bg-primary-500/10 border border-primary-200 dark:border-primary-800 rounded-xl p-3 mb-4 text-sm text-primary-800 dark:text-primary-300">
            <p>{t('השתמש בכל המילים')} <strong>{t('★ המסומנות בכחול')}</strong> {t('ובלפחות')} <strong>{t('6 מילים')}</strong> {t('מהרשימה.')}</p>
            <p className="text-xs text-primary-600 dark:text-primary-400 mt-0.5">{t('נושא')}: {wordList.theme}</p>
          </div>
          ...
          <div className="bg-surface rounded-2xl border border-card-border p-5 mb-4">
              ...
              <div className="flex gap-2">
                {sentence && (
                  <button onClick={() => { stopListening(); setSentence('') }}
                    className="text-xs text-fg/40 hover:text-red-500 px-2 py-1 rounded">{t('נקה')}</button>
                )}
                {speechSupported && (
            ...
            <textarea value={sentence} onChange={e => setSentence(e.target.value)}
              placeholder={t('כתוב כאן את המשפט שלך בעברית, או הקלט את עצמך...')}
            ...
            />
            {isListening && <p className="text-xs text-red-500 dark:text-red-400 mt-1.5 animate-pulse">{t('🎤 מקליט... לחץ ״עצור״ כשתסיים.')}</p>}
          </div>
          ...
          <div className={`rounded-2xl border p-5 text-center mb-4 ${scoreBg(feedback.score)}`}>
            ...
            <div className="mt-2 text-xs text-fg/60">
              {feedback.used_all_starred
                ? <span className="text-green-600 dark:text-green-400">{t('✓ כל מילות החובה שומשו')}</span>
                : <span className="text-red-500 dark:text-red-400">{t('✗ חסרו')}: {feedback.missing_starred.join(', ')}</span>}
              <span className="mr-2">&bull; {feedback.words_used_count} {t('מילים שומשו')}</span>
            </div>
          </div>
            ...
            <div className="bg-green-50 border border-green-200 dark:bg-green-950/40 dark:border-green-800 rounded-2xl p-5 mb-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <button onClick={playImproved} disabled={ttsLoading}
                  ...
                  <span>{ttsLoading ? t('טוען...') : t('האזן לגרסה המושלמת')}</span>
                </button>
              </div>
