...
export async function GET(req: NextRequest) {
  ...
  const [{ data: sessionAnswers }, { data: sessionOpenAnswers }, { data: sessionDebateAnswers }, { data: sessionRoleplayAnswers }] = s.kind === 'placement'
    ? [{ data: [] as { is_correct: boolean }[] }, { data: [] as { score: number }[] }, { data: [] as { score: number }[] }, { data: [] as { score: number }[] }]
    : await Promise.all([
        ...
        // and role-play (naale-roleplay-debate-parity) gaps this mirrors.
        db.from('naale_debate_answers').select('score').eq('session_id', s.id).eq('is_review', false),
        db.from('naale_roleplay_answers').select('score').eq('session_id', s.id).eq('is_review', false),
      ])
  const correct_count = (sessionAnswers ?? []).filter(a => a.is_correct).length
    + [...(sessionOpenAnswers ?? []), ...(sessionDebateAnswers ?? []), ...(sessionRoleplayAnswers ?? [])].filter(a => a.score >= 4).length
  ...
  return NextResponse.json({
    ...
    translation_lang: session.translationLang,
  })
}
