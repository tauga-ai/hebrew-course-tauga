...
export async function POST(req: NextRequest) {
  ...
  const [stale, overrideMinutes, { count: levelCount }, questionCounts, topicFlag] = await Promise.all([
    ...
    topic
      ? Promise.all([
          ...
          db.from('naale_roleplay_questions').select('question_id', { count: 'exact', head: true }).eq('topic', topic),
        ])
  ...
  if (topic) {
    const [{ count: mcqCount }, { count: openCount }, { count: debateCount }, { count: roleplayCount }] = questionCounts!
    if ((mcqCount ?? 0) === 0 && (openCount ?? 0) === 0 && (debateCount ?? 0) === 0 && (roleplayCount ?? 0) === 0) {
