...
export async function GET() {
  ...
  const [allTopics, allTopicsUnfiltered, { data: levels }, answers, openAnswers, debateAnswers, roleplayAnswers, sessions] = await Promise.all([
    ...
    // answer until it was fixed here. Role-play (naale-roleplay-debate-parity)
    // was closed proactively rather than repeating the same gap a second time.
    ...
    selectAll<{ topic: string; score: number; session_id: string; is_review: boolean }>('naale_roleplay_answers', (from, to) =>
      db.from('naale_roleplay_answers').select('topic, score, session_id, is_review').eq('student_id', studentId).range(from, to)),
  ...
  const progress = buildStudentProgress({
    ...
    levels: levels ?? [],
    answers,
    openAnswers: [...openAnswers, ...debateAnswers, ...roleplayAnswers],
    sessions,
  })
