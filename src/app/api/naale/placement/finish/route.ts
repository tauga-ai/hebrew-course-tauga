...
export async function POST(req: NextRequest) {
  ...
  const [{ data: placementAnswers }, { data: openPlacementAnswers }, allTopics] = await Promise.all([
    ...
    // absent (an absent row is what marks a student as unplaced). Shared,
    // paginated loader: a topic silently dropped here would leave a student
    // permanently unplaced in it.
    loadAllTopics(db),
  ])
