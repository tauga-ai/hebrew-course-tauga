...
- **Naale shared launch password** — `scripts/set-naale-password.ts` defaults every issued account to `Password123!`, which is published right here and therefore known: anyone with a rostered student's email address can sign in as them. Accepted for the Sept 1 launch window (no SMTP, so no self-serve reset); replace with per-student passwords or a forced first-login change once production SMTP exists. Staff rows refuse the default unless `--force-default` is passed, since a staff sign-in can read every student's data. **The email/password form itself is no longer dev-only** — it is production UI as of the `naale-password-login` ticket, per Idan's request that both sign-in options be available. See `.claude/ai-docs/docs/naale-password-login/issuing-credentials.md`.
- **`scripts/create-naale-test-users.ts`** and the 6 accounts it seeds (`naale_student1-3@test.com`, `naale_staff1-3@test.com`, password `Password123!`, documented in the gitignored `.claude/ai-docs/test-users.md`) — real rows in `naale_roster` + Supabase Auth. Delete these (roster rows + auth users) before a real demo/production use of that project.
- **`scripts/create-hebrew-course-test-users.ts`** and the 2 accounts it seeds (`hebrew_student_arabic@test.com`, `hebrew_student_russian@test.com`, password `Password123!`, documented in `.claude/ai-docs/test-users.md`) — real rows in `students` + Supabase Auth, one per existing draft-prep class. Created for live track-isolation testing (`.claude/ai-docs/tickets/naale-track-isolation/`). Delete these (students rows + auth users) before a real demo/production use of that project.
